...
const vaccineValidator = require('../validators/vaccination_type');
...
module.exports = {
  ...
  assignQuestionnaire: async (ctx) => {
    try {
      const { error, validatedData } = await vaccineValidator.validateAssignQuestionnaire(ctx.request.body);
      if (error) {
        ctx.res.unprocessableEntity({ error });
        return;
      }
