const alcoholValidator = require('../validators/alcohol');
...
const fhirSavve = async (alcoholList = []) => {
  try {
    const bundlePayload = {
      resourceType: 'Bundle',
      type: 'transaction',
      entry: alcoholList
        .filter(
          (alcoholObj) => (alcoholObj.status == 'Inactive' && alcoholObj.fhirId) || alcoholObj.status != 'Inactive'
        )
        .map((alcoholObj) => {
          alcoholObj.userFhirId = alcoholObj.userInfo.fhirId;
          const alcoholMappingData = mapFhirData.fhirMapping(alcoholObj);
          return {
            resource: alcoholMappingData,
            request: {
              method: alcoholObj.fhirId ? (alcoholObj.status == 'Inactive' ? 'DELETE' : 'PUT') : 'POST',
              url: `Observation${alcoholObj.fhirId ? `/${alcoholObj.fhirId}` : ''}`,
            },
          };
        }),
    };
    ...
    if (fhirFindListResp.data.entry) {
      fhirSyncArray = fhirSyncArray.map((fhirSyncObj) => {
        ...
        if (fhirFindData) {
          const alcoholFindData = alcoholList.find(
            (alcoholObj) => alcoholObj.uuid == fhirFindData.resource.identifier[0].value
          );
          if (alcoholFindData) {
            fhirSyncObj.id = alcoholFindData.id;
          }
        }
        return fhirSyncObj;
      });
      const alcoholDeletedArr = alcoholList.filter((alcoholObj) => alcoholObj.status == 'Inactive');
      fhirSyncArray = [
        ...fhirSyncArray,
        ...alcoholDeletedArr.map((alcoholObj) => ({
          id: alcoholObj.id,
          fhirSynced: true,
          deleted: true,
        })),
...
module.exports = {
  getById: async (ctx) => {
    try {
      const { error } = await alcoholValidator.validateGetById(ctx.request.params);
      if (error) {
  ...
  getList: async (ctx) => {
    try {
      const { error, validatedData } = await alcoholValidator.validateGetList(ctx.request.query);
      if (error) {
        return ctx.res.unprocessableEntity({ msg: error.message });
      }
  ...
  getChartData: async (ctx) => {
    try {
      const { error, validatedData } = await alcoholValidator.validateGetChartData(ctx.request.query);
      if (error) {
        return ctx.res.unprocessableEntity({ msg: error.message });
      }
  ...
  saveRecord: async (ctx) => {
    try {
      const { error } = await alcoholValidator.validateSave(ctx.request.body);
      if (error) {
        return ctx.res.unprocessableEntity({ msg: error.message });
      }
  ...
  updateRecord: async (ctx) => {
    try {
      const { error } = await alcoholValidator.validateUpdate(ctx.request.body);
      if (error) {
        return ctx.res.unprocessableEntity({ msg: error.message });
      }
  ...
  toggleArchiveRecord: async (ctx) => {
    try {
      const { error } = await alcoholValidator.validateArchive(ctx.request.body);
      if (error) {
        return ctx.res.unprocessableEntity({ msg: error.message });
      }
  ...
  deleteRecord: async (ctx) => {
    try {
      const { error } = await alcoholValidator.validateDelete(ctx.request.query);
      if (error) {
        return ctx.res.unprocessableEntity({ msg: error.message });
      }
